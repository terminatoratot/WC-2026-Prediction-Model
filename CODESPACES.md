...
```bash
V51_WORKERS=4 python backtest/eval_v51_knockout_walkforward.py
```

Results are written to `outputs/v51_walkforward/`. Parallelism is controlled
by `V51_WORKERS` in the command.
...
If `nproc` reports 32, start with:

```bash
V51_WORKERS=8 python backtest/eval_v51_knockout_walkforward.py
```

Do **not** set `V51_WORKERS=32`: every model fit already uses multiple CPU
threads internally, so 32 simultaneous fits will oversubscribe the machine
and can run slower or exhaust memory.
