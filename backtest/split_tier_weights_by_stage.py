...
group-stage vs. knockout matches, reusing the already-cached candidate rows
from outputs/v46_4_basev51_tier_optimization/cached_training_rows.csv --
no historical-card rebuilding (the expensive, ~3hr part) needed, since the
search itself operates purely on already-cached BetRow data.

Group-stage/knockout split is determined by joining match team-pairs against
data/fbref_world_cup_matches.csv's "round" column (same canon_team-based
matching used elsewhere this session), not by position in the file.
"""
...
CACHE_CSV = Path("outputs/v46_4_basev51_tier_optimization/cached_training_rows.csv")
...
def main() -> None:
    ...
    for label, outdir, subset in [
        ("group-stage", Path("outputs/v46_4_basev51_tier_optimization_group_stage"), df[~df["is_knockout"]]),
        ("knockout", Path("outputs/v46_4_basev51_tier_optimization_knockout"), df[df["is_knockout"]]),
    ]:
        bets = [
            BetRow(
