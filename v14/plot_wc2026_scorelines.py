#!/usr/bin/env python3
"""Plot actual WC2026 scores against the V14 model's two most likely scorelines.

Same chart style as compare_v11_top_scorelines.py, but driven by the v14 player
model (lineup variant, FC 26 player data) over the observed WC2026 group games in
data/wc2026_observed_matches_from_screenshots.csv. Predictions use the latest
player edition (FC 26) by leaving match_date unset.
"""
...
def build_comparison(args: argparse.Namespace) -> pd.DataFrame:
    ...
    print(f">>> building v14 model (variant={args.variant}) ...")
    model, _ = v14.build_from_zip(args.worldcupsai_zip, variant=args.variant)

    rows = []
    for observed_order, row in observed.iterrows():
        display_a, display_b = str(row["team_a"]), str(row["team_b"])
        ...
        first, second = score_text(top_one), score_text(top_two)
        rows.append(
            {
                ...
                "player_covered": bool(
                    prediction.get("player_adjustment", {}).get("covered")
                ),
                "team_a_win_probability": prediction["result_probabilities"]["team_a_win"],
                "draw_probability": prediction["result_probabilities"]["draw"],
                "team_b_win_probability": prediction["result_probabilities"]["team_b_win"],
            }
        )
...
def draw_scoreline_chart(comparison: pd.DataFrame, output_path: Path) -> None:
    """Draw a clean, compact scoreline comparison (V14)."""
    ...
    fig.text(
        0.06, 0.94,
        "Actual scores vs V14's two leading forecasts",
        fontsize=20, fontweight="bold", color=navy, va="top",
    )
    hits = int(comparison["actual_in_top_2"].sum())
    covered = int(comparison["player_covered"].sum())
    fig.text(
        0.06, 0.895,
        (
            f"{count} observed WC2026 matches   "
            f"Player-covered (FC 26): {covered}/{count}   "
    ...
    for x, label in headers:
        ax.text(x, y_header, label, fontsize=10, fontweight="bold", color=muted, va="center")
    ...
    for display_index, row in comparison.reset_index(drop=True).iterrows():
        ...
        ax.text(0.25, y + 0.10, f"{row.team_a} vs {row.team_b}",
                fontsize=11.4, fontweight="bold", color=navy, va="center")
        ax.text(0.25, y - 0.18, f"Match {int(row.observed_order)}",
                fontsize=8.7, color=muted, va="center")
        ...
        ax.text(5.55, y, row.actual_score, fontsize=15, fontweight="bold", color=navy,
                ha="center", va="center",
                bbox={"boxstyle": "round,pad=0.32", "facecolor": status_bg, "edgecolor": "none"})
        ...
        for x, score, probability, bar_color in [
            ...
            ax.text(x, y + 0.10, score, fontsize=13.5, fontweight="bold", color=navy, va="center")
            track_left = x + 0.78
            track_width = 1.75
            ax.barh(y + 0.08, track_width, height=0.12, left=track_left, color=pale_blue)
            ax.barh(y + 0.08, track_width * min(probability / 0.13, 1.0),
                    height=0.12, left=track_left, color=bar_color)
            ax.text(x, y - 0.22, f"{probability:.1%}", fontsize=9.3, color=muted, va="center")

        ax.text(14.55, y + 0.07, status_text, fontsize=10, fontweight="bold", color=status_color,
                ha="center", va="center",
                bbox={"boxstyle": "round,pad=0.3", "facecolor": status_bg, "edgecolor": "none"})
        ax.text(14.55, y - 0.23, f"Actual: {row.actual_score_probability:.1%}",
                fontsize=8.6, color=muted, ha="center", va="center")

    ax.text(0.25, -0.62,
            ("Coverage indicates whether the actual exact score ranked first, "
             "second, or outside V14's two most likely scorelines."),
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=["lineup", "squad", "none"], default="lineup")
    parser.add_argument("--matches", type=int, default=12,
                        help="Use the first N observed rows (default: 12 = all).")
    parser.add_argument("--output-dir",
                        default=str(Path(__file__).resolve().parent / "wc2026_scoreline_chart"))
    ...
    csv_path = output_dir / f"v14_top_two_scoreline_comparison_{suffix}.csv"
    plot_path = output_dir / f"v14_top_two_scoreline_comparison_{suffix}.png"
